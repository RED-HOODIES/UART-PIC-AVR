...
# 2 "<built-in>" 2
# 1 "PARTE3_PARCIAL2.c" 2
# 11 "PARTE3_PARCIAL2.c"
...
void UART_init_baud(const long int baudRate)
{
    ...
    if(valor>255)
    {
        valor = (unsigned char)((16000000/(baudRate*64)) -1);
        TXSTAbits.BRGH = 0;
    }
...
void LCD_Write(char value, char mode) {
    ...
    LATAbits.LATA2 = 1; _delay((unsigned long)((1)*(16000000/4000000.0))); LATAbits.LATA2 = 0;;
    if (mode) {
...
void IMPRIMIR(){
    ...
    LCD_Text("ADRES: ");
    while(LCD_Busy()); LATAbits.LATA3 = 0; LCD_Write(0x80 + (0x40 * 0) + 7,1);
    LCD_Text(imprimir);

    sprintf(imprimir2, "%.2f", VOLTAJE(ADC));
    while(LCD_Busy()); LATAbits.LATA3 = 0; LCD_Write(0x80 + (0x40 * 1) + 0,1);
    LCD_Text("VOLTAJE: ");
    while(LCD_Busy()); LATAbits.LATA3 = 0; LCD_Write(0x80 + (0x40 * 1) + 10,1);
    LCD_Text(imprimir2);
